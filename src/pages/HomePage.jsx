...
const testimonials = [
  { name: 'Rafi, Jakarta', text: 'Proses cepat dan aman.', photo: prod1 },
  { name: 'Nadia, Bandung', text: 'Checkout gampang banget.', photo: prod2 },
  { name: 'Bayu, Surabaya', text: 'Worth it buat mulai.', photo: prod3 },
]
...
export default function HomePage() {
  ...
  return <main>
    ...
    <section className="section"><div className="filters">{['all','ai','editing','token'].map(c=><button key={c} className={`filter ${activeCategory===c?'active':''}`} onClick={()=>setActiveCategory(c)}>{c==='all'?'Semua':c.toUpperCase()}</button>)}</div>
    <div className="grid">{filteredProducts.map(p=><article className="card" key={p.id}><img src={p.image}/><div className="card-body"><span className="tag">{p.tag}</span><h3>{p.name}</h3><p className="desc">{p.desc}</p><div className="price-wrap"><span className="old-price">Rp{p.oldPrice.toLocaleString('id-ID')}</span><span className="discount">Diskon 60%</span></div><p className="price">Rp{p.price.toLocaleString('id-ID')}</p><button className="btn" onClick={()=>setSelected(p)}>Beli Sekarang</button></div></article>)}</div></section>
    <section className="section testimonials"><h2>Testimoni</h2><div className="marquee"><div className="marquee-track">{[...testimonials,...testimonials].map((t,i)=><article key={i} className="testi-card"><img src={t.photo} /><p>“{t.text}”</p><strong>{t.name}</strong></article>)}</div></div></section>
